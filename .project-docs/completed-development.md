# Completed items during development phase
...
- Added Client Membership validation during authorization-code and refresh-token redemption.
- Rejected token redemption when the user, client application, or current Client Membership can no longer be resolved.
- Reused the shared client-membership resolution service across interactive authorisation and token redemption.
- Refreshed user identity and application-role claims before issuing replacement tokens.
- Preserved OpenIddict token metadata, lifetime, rotation, scope, and resource behaviour.
- Confirmed that existing access tokens remain valid until expiry while further token issuance is blocked immediately after membership removal.
- Verified membership removal and application-role changes across authorization-code exchange, access-token use, and refresh-token renewal.
